	...
	for (corps_it = my_Corps.begin(); corps_it != my_Corps.end(); corps_it++) {//遍历我的兵团，找出他们的种类
		...
		if (info.corpsInfo[i].exist) {
			...
			if (info.corpsInfo[i].type == Battle) {
				if (info.corpsInfo[i].m_BuildType == Warrior) {
					my_Warrior_num++;
					my_Warrior.push_back(i);
				}
				if (info.corpsInfo[i].m_BuildType == Archer) {
					my_Archer_num++;
					my_Archer.push_back(i);
				}
				if (info.corpsInfo[i].m_BuildType == Cavalry) {
					my_Cavalry_num++;
...
//对作战兵团下命令
	for (int j = 0; j < my_Warrior_num; j++) {
		...
		int x = info.corpsInfo[q].pos.m_x;
		int y = info.corpsInfo[q].pos.m_y;
		for (int dx = 0; dx < TBattleRange[0]; dx++) {
			for (int dy = 0; dy < TBattleRange[0]; dy++) {
				if (int p = canfight(info, q, x + dx, y + dy, myID)) {
					info.myCommandList.addCommand(corpsCommand, { CAttackCorps,myID, p });					command_num++;				}
			}
		}
	}
	for (int j = 0; j < my_Archer_num; j++) {
		...
		int x = info.corpsInfo[q].pos.m_x;
		int y = info.corpsInfo[q].pos.m_y;
		for (int dx = 0; dx < TBattleRange[1]; dx++) {
			for (int dy = 0; dy < TBattleRange[1]; dy++) {
				if (int p = canfight(info, q, x + dx, y + dy, myID)) {
					info.myCommandList.addCommand(corpsCommand, { CAttackCorps,myID, p });					command_num++;				}
			}
		}
		...
		if (!flag) {
			for (int dx = -2; dx < 2; dx++) {//如果敌方兵团在攻击范围内，则攻打
				for (int dy = -2; dy < 2; dy++) {
					if (int p = canfight(info, i, x + dx, y + dy, myID)) {
